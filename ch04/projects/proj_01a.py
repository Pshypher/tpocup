...
codemaker_seq = input("Enter 4 uppercase alphabets, either one of(ABCDEF): ")
codemaker_seq = codemaker_seq.upper()
valid_color_code = False

# Validate the string entered by the code-master
while not valid_color_code:
    for i, ch in enumerate(codemaker_seq):
        if ch not in COLORS:
            print(ch, "isn't one of \"ABCDEF\"")
            codemaker_seq = input("Enter 4 uppercase alphabets, either one of(ABCDEF): ")
            codemaker_seq = codemaker_seq.upper()
            break
    else:
        if i != CODE_LENGTH-1:   
            print("the length of the code ought to be 4, try again")
            codemaker_seq = input("Enter 4 uppercase alphabets, either one of(ABCDEF): ")
            codemaker_seq = codemaker_seq.upper()
        else:
